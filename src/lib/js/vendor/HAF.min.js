...
return i.apply(this,arguments);};};var d;for(d in f){if(f.hasOwnProperty(d)){var e=f[d],g=c.prototype[d];c.prototype[d]=!a(e)||!a(g)?e:h(e,g);}}c.prototype.constructor=c;c.extend=this.extend;return c;};window.Class=b;}());(function(b){b.Base=Class.extend({_guid:null,messageBus:{publish:f,subscribe:f,unsubscribe:f},guid:function(){if(!this._guid){this._guid=c();
}return this._guid;},injectDependencies:function(h){g(this,h);}});b.noop=f;b.each=function(h,i){if(h){if(h instanceof Array){e(h,i);}else{d(h,i);}}};function d(h,j){var i;if(h){for(i in h){if(h.hasOwnProperty(i)){j(h[i],i);}}}}function e(k,m){var j,h;if(k){for(j=0,h=k.length;j<h;j++){m(k[j],j);}}}function g(h,i){if(b.Messaging&&(i instanceof b.Messaging)){h.messageBus=i;
}var j=(i&&i.inject)||(h.inject&&h.inject());b.each(j,function(k,l){h[l]=k;});}function c(){return a()+a()+"-"+a()+"-"+a()+"-"+a()+"-"+a()+a()+a();}function a(){return Math.floor((1+Math.random())*65536).toString(16).substring(1);}function f(){}}(HAF));(function(g){g.Controller=g.Base.extend({autoWire:false,autoDestroy:false,autoShowHide:false,autoLoadControls:false,autoLayout:false,messages:null,inject:null,requireMessageBus:false,routes:{},serviceUpdate:{},init:function(s){if(this.requireMessageBus){this.messageBus=new g.Messaging();
}this.injectDependencies(s);},views:null,templates:null,controls:null,services:null,layoutChange:function(){if(this.autoLayout){p(this.controls,"layoutChange");p(this.views,"layoutChange");}},load:function(){k(this);a(this);},unload:function(){j(this);e(this);},update:function(s){c(this,s);},onUpdateReceive:function(t,s){f(this,t,s);
...
}(HAF));(function(b,c){b.View=b.Base.extend({autoBindManagement:false,autoLayout:false,init:function(e){this.injectDependencies(e);this.$container=c(this.container);this.$el=this.$container.$item;if(!this.autoBindManagement){this.bind();}},container:null,$container:null,bindings:null,layoutChange:b.noop,bind:function(){d(this);
},unbind:function(){a(this);},$el:null,render:function(e){this.$container.html(e);},destroy:function(){this.$container.empty();},hide:function(){var e=this;e.$el.hide();if(e.autoBindManagement){e.unbind();}if(e.autoLayout){c(window).off("resize."+e.guid());}},show:function(){var e=this;e.$el.show();if(e.autoBindManagement){e.bind();
}if(e.autoLayout){c(window).off("resize."+e.guid()).on(("resize."+e.guid()),function(){e.layoutChange();});}}});function a(e){b.each(e.bindings,function(g,f){var h=/([a-z]+)\s([a-zA-Z0-9\-\.\(\)>]+)/.exec(f);e.$container.off(h[1],h[2]);});}function d(e){var f=typeof e.bindings==="function"?e.bindings():e.bindings;
